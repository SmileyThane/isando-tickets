...
class EmailReceiverRepository
{
    ...
    public function receiveMail()
    {
        ...
        try {
            ...
            $this->handleEmailMessages($messages);
    ...
    private function handleEmailMessages($messages)
    {
        $responseBody = null;
            ...
            if ($userGlobal) {
                ...
                try {
                    ...
                    if (
                        ...
                        $cachedCount === 0 &&
                        (in_array($senderEmail, MailCache::CONTACT_FORM_ADDRESSES, true) || $ticket === null)
                    ) {
                ...
                if ($cachedCount === 0) {
                    ...
                    if ($paramsArray) {
                        ...
                        $responseBody = $this->createTicketFromEmail(
                            $senderEmail,
                            $generatedMessage['description'],
                            $ticketSubject,
                            $attachments,
                            $priorityId,
                            $generatedMessage['access_details']
                        );
                    }
                }
            }
    ...
    private function ticketAnswerFromEmail($senderEmail, $ticket, $message, $files = []): bool
    {
        ...
        $params = new Request();
        $params['answer'] = $message->hasHTMLBody() ?
            $this->removeEmptyParagraphs($message->getHTMLBody(true)) : $message->getTextBody();
        $params['files'] = $files;
    ...
    ): ?Ticket
    {
        ...
        if ($fromEntityId && $fromEntityType && $toEntityId && $toEntityType && $productId) {
            ...
            if (is_string($message)) {
                ...
                $params['description'] = $message->hasHTMLBody() ?
                    $this->removeEmptyParagraphs($message->getHTMLBody(true)) : $message->getTextBody();
            }
            $params['files'] = $files;
            return $this->ticketRepo->create($params, $userFrom->employee->id);
        }
        Log::alert("check incorrect params fromEntityId=$fromEntityId  fromEntityType=$fromEntityType
         toEntityId=$toEntityId toEntityType=$toEntityType productId=$productId");
        return null;
    }
    ...
    private function createDescriptionFromParsedArray($parsedArray): array
    {
        ...
        $message['description'] .= '<p><strong>From: </strong>' . $parsedArray['From / Name'] . ' - ' .
            $parsedArray['Firm / Organisation'] . ' sent on ' . now()->format('d-m-Y h:i:s') . "</p>";
        ...
        $message['description'] .= '<p><strong>Briefly describe your problem: </strong>' .
            str_replace("\n", '<br/>', $parsedArray['Briefly describe your problem']) . "</p>";

        $message['access_details'] .= 'Software/Hardware affected: ' .
            $parsedArray['Software/Hardware affected'] . "<br>";
