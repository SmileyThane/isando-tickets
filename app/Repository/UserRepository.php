    ...
    public function sendInvite($user, $role, $password = null): bool
    {
        ...
        try {
            if ($password === null) {
                ...
                $user->save();
                $user->notify(new ResetPasswordEmail($from, $user->full_name, $role, $user->email, $password));
            } else {
                $user->notify(new RegularInviteEmail($from, $user->full_name, $role, $user->email, $password));
            }
