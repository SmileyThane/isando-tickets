...
class ChangedFollowTicketStatus extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable): MailMessage
    {
        ...
        if ($this->language == 'en') {
            ...
            if ($ticket && $ticket->status_id === 5) {
                $subject = 'Updates on follow ticket: ' . $this->ticket_subject;
                $firstLine = "Ticket has been successfully closed.
             We hope that you have been satisfied with the resolution of the ticket and the speed of response. ";
                $secondLine = "";
            } else {
                $subject = 'Updates on follow ticket: ' . $this->ticket_subject;
                $firstLine = "Ticket has been updated.";
