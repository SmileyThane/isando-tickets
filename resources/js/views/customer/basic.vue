    ...
    <v-row>
      <v-col cols="6">
        <v-card>
          ...
          <v-card-text v-if="!enableToEdit">
            <v-row>
              ...
              <v-col :cols="client.logo_url ? 4 : 6">
                ...
                <p v-if="client.client_description">
                ...
                >
                  ...
                  <p v-for="item in client.emails" :key="item.id" class="mb-0">
                    <v-icon
                      v-if="item.type"
                      :title="$helpers.i18n.localized(item.type)"
                      class="mr-2"
                      dense
                      small
              ...
              <v-col cols="6">
                ...
                >
                  <p
                    v-for="item in client.addresses"
                    :key="item.id"
                    class="mb-1"
                  >
                    <v-icon
                    ...
                    <br />{{ item.postal_code }} {{ item.city }} <br /><span
                      v-if="item.country"
                      >{{ $helpers.i18n.localized(item.country) }}</span
                    >
                  </p>
                </div>
          ...
          <v-expand-transition>
            <v-card v-if="enableToEdit" class="transition-fast-in-fast-out">
                ...
                <v-form>
                  <v-row>
                    <v-col cols="2">
                      ...
                      <v-img
                        :src="logo"
                        style="z-index: 1; max-height: 80px; max-width: 80px"
                      >
                        <v-file-input
                          v-model="newLogo"
                          accept="image/*"
                  ...
                  <v-row>
                    <v-col cols="6">
                      <v-list-item v-for="item in client.emails" :key="item.id">
                        ...
                        <v-list-item-action>
                          <v-icon small @click="editEmail(item)"
                            >mdi-pencil</v-icon
                          >
                        </v-list-item-action>
                    ...
                    <v-col cols="6">
                      <v-list-item v-for="item in client.phones" :key="item.id">
                        ...
                        <v-list-item-action>
                          <v-icon small @click="editPhone(item)"
                            >mdi-pencil</v-icon
                          >
                        </v-list-item-action>
                        <v-list-item-action>
                          <v-icon small @click="deletePhone(item.id)"
                            >mdi-delete</v-icon
                          >
                        </v-list-item-action>
                      </v-list-item>
                  ...
                  <v-row>
                      ...
                      >
                        ...
                        <v-list-item-content>
                          <v-list-item-title v-text="">
                            <span v-if="item.street">{{ item.street }}</span>
                            <span v-if="item.street2"
                              >, {{ item.street2 }}</span
                            >
                            <span v-if="item.street3"
                              >, {{ item.street3 }}</span
                            >
                            <br />{{ item.postal_code }}&nbsp;&nbsp;{{
                              item.city
                        ...
                        <v-list-item-action>
                          <v-icon small @click="editAddress(item)"
                            >mdi-pencil</v-icon
                          >
                        </v-list-item-action>
                        <v-list-item-action>
                          <v-icon small @click="deleteAddress(item.id)"
                            >mdi-delete</v-icon
                          >
                        </v-list-item-action>
                      </v-list-item>
                    ...
                    <v-col cols="6">
                      ...
                      >
                        ...
                        <v-list-item-action>
                          <v-icon small @click="editSocial(item)"
                            >mdi-pencil</v-icon
                          >
                        </v-list-item-action>
                        <v-list-item-action>
                          <v-icon small @click="deleteSocial(item.id)"
                            >mdi-delete</v-icon
                          >
                        </v-list-item-action>
                      </v-list-item>
              ...
              <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="grey darken" text @click="cancelUpdateClient">
                  {{ langMap.main.cancel }}
                </v-btn>
                <v-btn :color="themeBgColor" text @click="updateClient">
                  {{ langMap.main.save }}
                </v-btn>
              </v-card-actions>
            </v-card>
        ...
        <v-card>
          ...
          <v-card-text>
                  ...
                  <v-tooltip top>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn
                        ...
                        v-on="on"
                        @click.native.stop="
                          removeEmployeeProcess(item.employee)
                        "
                      >
                ...
                <v-card>
                  ...
                  <v-card-actions>
                    ...
                    <v-btn
                      color="grey darken-1"
                      text
                      @click="removeEmployeeDialog = false"
                    >
                      {{ langMap.main.cancel }}
                    </v-btn>
                    <v-btn
                      color="red darken-1"
                      text
                      @click="removeEmployee(selectedEmployeeId)"
                    >
                      {{ langMap.main.delete }}
                    </v-btn>
                  </v-card-actions>
                ...
                <v-card>
                  ...
                  <v-card-actions>
                    ...
                    <v-btn
                      color="grey darken-1"
                      text
                      @click="unlinkEmployeeDialog = false"
                    >
                      {{ langMap.main.cancel }}
                    </v-btn>
                    <v-btn
                      color="red darken-1"
                      text
                      @click="unlinkEmployee(selectedEmployeeId)"
                    >
                      {{ langMap.main.delete }}
                    </v-btn>
                  </v-card-actions>
            ...
            <v-dialog
              v-if="contactInfoForm !== null"
              v-model="contactInfoModal"
              max-width="600"
            >
              <v-card>
                <v-card-title
                ...
                <v-card-text>
                  ...
                  <v-row>
                    <span class="col-md-6 pa-4 text-left pt-0">
                      <br />
                      <span>
                        {{ langMap.main.email }}
                      </span>
                    ...
                    <v-spacer></v-spacer>
                    <span class="col-md-6 pa-4 pt-0 text-right">
                      <br />
                      <span>
                        {{ langMap.main.email }}
                      </span>
                      <br />
                      <strong
                        v-for="email in contactInfoForm.employee.user_data
                          .emails"
                      >
                        <span>
                          {{ email.email }}
                      ...
                      <br />
                      <strong
                        v-for="phone in contactInfoForm.employee.user_data
                          .phones"
                      >
                        <span>
                          {{ phone.phone }}
                ...
                <v-card-actions>
                  ...
                  <v-btn color="green darken-1" text @click="editContactInfo">
                    <v-icon small>{{
                      contactInfoEditBtn === false ? 'mdi-pencil' : ''
                    }}</v-icon>
                    ...
                    <div class="row">
                      ...
                      <div class="col-md-6">
                        ...
                        <v-checkbox
                          ...
                          :disabled="
                            $helpers.auth.checkPermissionByIds(['36,37'])
                          "
                    ...
                    <div class="row">
                      <v-col class="pa-1" cols="md-12">
                        <v-autocomplete
                          ...
                          :items="employees"
                          :placeholder="
                            this.$store.state.lang.lang_map.main.search
                          "
                          hide-no-data
                          hide-selected
                          item-text="user_data.full_name"
                      ...
                      <v-text-field
                        ...
                        :color="themeBgColor"
                        :label="
                          this.$store.state.lang.lang_map.main.description
                        "
                        class="pa-1"
                        dense
                        type="text"
                      ...
                      <v-form>
                        <v-row>
                          ...
                          <v-btn
                            ...
                            small
                            @click="
                              updateInternalBilling(internalBillingForm.id)
                            "
                          >
                            <v-icon
                              :color="themeBgColor"
      ...
      <v-col cols="6">
                ...
                <v-expansion-panel-content>
                  <v-form>
                    <div class="row">
                      ...
                      <v-col cols="md-6">
                        ...
                        >
                          ...
                          <v-time-picker
                            ...
                            full-width
                            @click:minute="
                              $refs.menuActivityTimeRef.save(activityForm.time)
                            "
                          ></v-time-picker>
                        </v-menu>
                      </v-col>
                      ...
                      >
                        <h4>{{ langMap.main.attachments }}</h4>
                        <div v-for="attachment in activityForm.attachments">
                          <v-chip
                            :color="themeBgColor"
                            :href="attachment.link"
                            :text-color="themeFgColor"
        ...
        <v-card>
          ...
          <v-card-text>
            <Tinymce v-model="client.notes" :placeholder="langMap.main.notes" />
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
        ...
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn
              ...
              text
              @click="createOrRestoreEmployee('create')"
            >
              {{ langMap.main.create }}
            </v-btn>
            <v-btn
              color="blue darken-1"
              text
              @click="createOrRestoreEmployee('restore')"
            >
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        {
          text: `${this.$store.state.lang.lang_map.company.user}`,
          value: 'user_data',
        },
        {
          text: `${this.$store.state.lang.lang_map.main.roles}`,
      ...
      contactHeaders: [
        ...
        {
          text: `${this.$store.state.lang.lang_map.main.phone}`,
          value: 'employee.user_data.phones[0].phone',
        },
        {
          text: `${this.$store.state.lang.lang_map.main.role}`,
          value: 'employee.role_names',
        },
        {
          text: `${this.$store.state.lang.lang_map.main.actions}`,
      ...
      activityHeaders: [
        ...
        {
          text: `${this.$store.state.lang.lang_map.tracking.tracker.date}`,
          value: 'datetime',
        },
        {
          text: `${this.$store.state.lang.lang_map.main.type}`,
          value: 'type.name',
        },
        {
          text: `${this.$store.state.lang.lang_map.main.updated}`,
          value: 'updated_at',
        },
        {
          text: `${this.$store.state.lang.lang_map.main.actions}`,
  ...
  methods: {
    ...
    getEmployees() {
      axios
        .get('/api/employee?sort_by=user_data.name&sort_val=false')
        .then((response) => {
          response = response.data;
          if (response.success === true) {
    ...
    addEmployee(update = false) {
      axios.post(`/api/client/employee`, this.employeeForm).then((response) => {
        ...
        if (response.success === true) {
          ...
          this.getClient();
          this.snackbarMessage = update
            ? this.langMap.company.employee_updated
            : this.langMap.company.employee_created;
          ...
          if (response.error.email_trashed) {
            ...
            let error = Object.keys(response.error)
              ? response.error[Object.keys(response.error)[0]].shift()
              : this.langMap.main.generic_error;
    ...
    editContactInfo() {
      if (this.contactInfoEditBtn === true) {
        ...
        this.employeeForm.company_user_id =
          this.contactInfoForm.company_user_id;
    ...
    updateClient() {
      ...
      if (this.newLogo) {
        ...
        axios
          .post(`/api/client/${this.$route.params.id}/logo`, formData, {
      ...
      this.client.supplier_type = Object.keys(
        this.client.supplier_object
      ).shift();
      this.client.supplier_id = Object.values(
        this.client.supplier_object
      ).shift();
      console.log(this.client);
      this.client.filter_groups = this.client_groups?.map(
        (group) => group.name
      );
      axios
        .patch(`/api/client/${this.$route.params.id}`, this.client)
        .then((response) => {
          response = response.data;
          if (response.success === true) {
            this.getClient();
    ...
    saveNote() {
      axios
        .patch(`/api/client/${this.$route.params.id}/notes`, {
    ...
    updatePhone() {
      ...
      axios
        .patch(`/api/phone/${this.phoneForm.id}`, this.phoneForm)
        .then((response) => {
          response = response.data;
          if (response.success === true) {
            this.resetPhone();
    ...
    updateSocial() {
      ...
      axios
        .patch(`/api/social/${this.socialForm.id}`, this.socialForm)
        .then((response) => {
          response = response.data;
          if (response.success === true) {
            this.resetSocial();
    ...
    updateAddress() {
      ...
      axios
        .patch(`/api/address/${this.addressForm.id}`, this.addressForm)
        .then((response) => {
          response = response.data;
          if (response.success === true) {
            this.resetAddress();
    ...
    changeIsActiveClient(item) {
      ...
      axios.post(`/api/client/is_active`, request).then((response) => {
        ...
        if (response.success === true) {
          ...
          this.snackbarMessage =
            item.is_active === true
              ? this.langMap.company.company_activated
              : this.langMap.company.company_deactivated;
          this.actionColor = 'success';
          this.snackbar = true;
        } else {
          this.snackbarMessage = this.langMap.main.generic_error;
    ...
    showSupplier() {
      let to =
        this.client.supplier_type === 'App\\Client' ? 'customer' : 'company';
    ...
    changeIsActive(item) {
      ...
      axios.post(`/api/user/is_active`, request).then((response) => {
        ...
        if (response.success === true) {
          this.getClient();
          this.snackbarMessage = item.is_active
            ? this.langMap.company.employee_activated
            : this.langMap.company.employee_deactivated;
          this.actionColor = 'success';
          this.snackbar = true;
        } else {
          this.snackbarMessage = this.langMap.main.generic_error;
    ...
    resetPassword(item) {
      axios
        ...
        .then((response) => {
          ...
          if (response.success === true) {
            this.snackbarMessage =
              this.langMap.company.reset_password_email_sent;
            this.actionColor = 'success';
            this.snackbar = true;
          } else {
            this.snackbarMessage = this.langMap.main.generic_error;
    ...
    updateEmail() {
      ...
      axios
        .patch(`/api/email/${this.emailForm.id}`, this.emailForm)
        .then((response) => {
          response = response.data;
          if (response.success === true) {
            this.emailForm.id = '';
    ...
    addActivity() {
      ...
      if (this.activityForm.files) {
        Array.from(this.activityForm.files).forEach((file) =>
  ...
  watch: {
    ...
    clientUpdates() {
      ...
      if (this.singleUserForm.user) {
        this.singleUserForm.user = this.client.employees.find(
          (x) => x.employee.user_id === this.singleUserForm.user.id
        ).employee.user_data;
      }
    },
    newLogo() {
