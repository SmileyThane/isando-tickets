...
export default {
      ...
      switch (tab) {
        ...
        case 7: {
          return (this.options = {
            ...options,
            ...emptyOptions,
            status: this.ticketStatuses
              .filter((status) => status.id === 5)
  ...
  beforeMount() {
    this.$router.push({
      name: this.pageName,
      query: { ...this.$route.query, tab: this.tab, page: 1 },
    });
  },
  watch: {
    activeTab(value) {
    ...
    tablePage: function (value) {
      this.$router.push({
        name: this.pageName,
        query: { ...this.$route.query, page: value, tab: this.activeTab },
      });
      // this.getTickets();
    },
  },
