<template>
  <v-navigation-drawer v-model="drawer" :mini-variant.sync="localDrawer" app>
    <perfect-scrollbar style="height: 100%">
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              <v-list-item
                ...
                v-on="on"
                v-if="
                  $helpers.auth.checkKbPermissionsByType(item.alias, 'view')
                "
                :key="index"
                style="background-color: white"
                dense
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title v-text="$helpers.i18n.localized(item)">
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
      ...
      <v-divider></v-divider>
      <v-list v-if="$helpers.auth.checkPermissionByIds([100])" dense>
        <v-list-group
          :style="'background-color: ' + 'white' + ';'"
          :value="sidebarGroups"
          color="#757575"
        ...
        >
          <template v-slot:activator>
            ...
            <v-list-item-content>
              <v-list-item-title
                >{{ langMap.sidebar.incident_reporting }}
              </v-list-item-title>
            </v-list-item-content>
          </template>
          <v-tooltip right :disabled="!localDrawer">
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.incident_reporting_scenarios }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.incident_reporting_action_boards }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
      ...
      <v-divider v-if="$helpers.auth.checkPermissionByIds([100])"></v-divider>
      <v-list dense>
        <v-tooltip right :disabled="!localDrawer">
          <template v-slot:activator="{ on, attrs }">
            <v-list-item
      ...
      <v-divider v-if="$helpers.auth.checkPermissionByIds([25])"></v-divider>
      <v-list dense>
        <v-list-group
        ...
        >
          ...
          <v-list-group :value="true" color="#757575" no-action sub-group>
            <template v-slot:activator>
              <v-list-item-content>
            ...
            <v-tooltip right :disabled="!localDrawer">
              <template v-slot:activator="{ on, attrs }">
                <v-list-item
                  ...
                  link
                  style="background-color: white"
                  to="/customer?type=clients"
                >
                  <v-list-item-action>
                    <v-icon>mdi mdi-bulletin-board</v-icon>
                  </v-list-item-action>

                  <v-list-item-content>
                    <v-list-item-title
                      >{{ langMap.sidebar.customers }}
                    </v-list-item-title>
                  </v-list-item-content>
                </v-list-item>
              </template>
      ...
      <v-list
        ...
        v-if="
          hasLicense &&
          $helpers.auth.checkPermissionByIds([
            40, 41, 42, 51, 52, 54, 55, 61, 66,
          ])
        "
      >
        <v-list-group
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.tracking_dashboard }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              <v-list-item
                ...
                v-on="on"
                v-if="
                  hasLicense && $helpers.auth.checkPermissionByIds([40, 41, 42])
                "
                link
                style="background-color: white"
                to="/tracking/tracker"
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.tracking_tracker }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
                ...
                <v-list-item-content>
                  <v-list-item-title>
                    <v-badge
                      v-if="
                        $store.getters[
                          'Timesheet/getCountTimesheetForApproval'
                        ] > 0
                      "
                      :color="themeBgColor"
                      class="mt-0"
                      :content="
                        $store.getters['Timesheet/getCountTimesheetForApproval']
                      "
                      inline
                    >
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.tracking_calendar }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              <v-list-item
                ...
                v-on="on"
                v-if="
                  hasLicense && $helpers.auth.checkPermissionByIds([54, 55])
                "
                link
                style="background-color: white"
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              <v-list-item
                ...
                v-on="on"
                v-if="
                  hasLicense && $helpers.auth.checkPermissionByIds([61, 66])
                "
                link
                style="background-color: white"
                to="/tracking/reports"
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.tracking_reports }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              <v-list-item
                ...
                v-on="on"
                v-if="
                  hasLicense && $helpers.auth.checkPermissionByIds([75, 79])
                "
                link
                style="background-color: white"
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.tracking_settings }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.notify_customers }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.notifications_history }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
      ...
      <v-divider v-if="$helpers.auth.checkPermissionByIds([22])"
        >&nbsp;
      </v-divider>
      <v-list dense>
        <v-list-group
          ...
          <v-tooltip right :disabled="!localDrawer">
            <template v-slot:activator="{ on, attrs }">
              ...
              >
                ...
                <v-list-item-content>
                  <v-list-item-title
                    >{{ langMap.sidebar.system_settings }}
                  </v-list-item-title>
                </v-list-item-content>
              </v-list-item>
            </template>
...
export default {
  ...
  methods: {
    ...
    getCompanySettings() {
      axios.get(`/api/main_company/settings`).then((response) => {
        response = response.data;
        if (response.success === true) {
          this.navbarStyle = response.data.hasOwnProperty('navbar_style')
            ? response.data.navbar_style
            : 1;
        }
      });
    },
    changeAppTitle() {
      ...
      document.title =
        this.firstAlias + ' | ' + this.langMap.sidebar[this.$route.name];
    },
    getTrackingSettings() {
  ...
  computed: {
    ...
    enableTimesheet() {
      return (
        this.$store.getters['Tracking/getSettings'].settings &&
        this.$store.getters['Tracking/getSettings'].settings.enableTimesheet
      );
    },
    getTrackingProjectLabel() {
      const { settings } = this.$store.getters['Tracking/getSettings'];
      const projectType =
        settings && settings.projectType ? settings.projectType : 0;
      switch (projectType) {
        case 1:
